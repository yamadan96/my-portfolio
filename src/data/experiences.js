...
const experiences = [
  ...
  },
  {
    ...
    top: {
      ...
      roleLine: '鉄パイプ計数 PoC でエンジニア兼テックリード',
    ...
    summary: {
      ...
      role: 'AIエンジニアとして複数の企業向けAI開発を担当。モデル選定・データ設計・学習・評価から、API化・業務自動化・顧客報告まで一貫して担当。鉄パイプ自動計数PoCではエンジニア兼テックリードを担当。',
    ...
    details: {
      ...
      highlights: [
        { axis: 'Computer Vision', title: '鉄パイプ自動計数', text: '束単位の検出成功率 96.6%（社内評価）、推論 0.2秒/枚。エンジニア兼テックリードを担当。' },
      ...
      projects: [
        {
          ...
          icon: '🤖',
          summary:
            '2026年9月からロボット部門に参画。カメラ画像・関節角度・言葉の指示から次の動作を出すモデル（VLA）による制御に取り組んでいる。',
          details: [
            '産業用マニピュレータを対象に、ROS 2 上でのシミュレーション操作と、順運動学・逆運動学といった制御の基礎から着手している。実機はまだ触っていない。',
        ...
        },
        {
          ...
          details: [
            ...
            '生成した要約・フローチャートの品質を CI/CD で自動判定するための評価方式を設計・検証。人手で付けた正解ラベル80件で比べ、LLM-as-a-Judge による判定（正解率97.5%）をベクトルの類似度による判定（78.8%）より適した方式として推奨した。',
          ],
  ...
  },
  {
    ...
    details: {
      ...
      responsibilities: [
        ...
        '【類型説明文の生成】説明文が空欄だった78類型（日本語45・英語33）の説明文を、法務チームが書いた16件を手本にする方式で生成。3類型で2つの方式を比べ、手本方式を採用',
      ],
      achievements:
  ...
  },
  {
    ...
    summary: {
      ...
      tech: '表形式データに強い機械学習（LightGBM）を使用。予測の自信度を3段階に分けて出力を振り分け、自信度が低い予測だけを補正した。自信度ごとに別のモデルを使う案も試したが、1つのモデルの方が精度が高かった。',
      result: '承認されるかの予測は精度95.7%・AUC 0.940（社内データのテスト2万件）。承認月の予測は正解率92.80%（テスト1万件）。予測結果を毎週自動で表計算ソフトへ反映する仕組みまで構築した。',
    },
    secondary: true,
    company: 'GMOメディア株式会社',
    role: '機械学習エンジニア',
    ...
    description:
      'ポイントサイトの売上を予測するAIモデルを構築し、精度95.7%（社内データのテスト2万件）。予測結果を毎週表計算ソフトへ自動反映する仕組みも構築。',
    ...
    details: {
      overview:
        ...
        'BigQueryによる大規模データ抽出から特徴量エンジニアリング、LightGBMアンサンブル学習、3段階の信頼度スコアによる出力の振り分け、' +
        'BigQuery→スプレッドシートへの週次自動反映パイプラインまで一貫して設計・実装し、最終日の成果発表会で業務導入を提案した。',
      responsibilities: [
        '【承認/否認予測モデル構築】LightGBMのアンサンブル学習で、テスト2万件に対し精度95.7%・AUC 0.940・F1 0.976。予測時点では分からない承認日・承認状態・経過日数の列が特徴量に入っていた（データリーク）ため除外し、申込時点で取得できる情報だけで学習し直した',
        '【特徴量エンジニアリング】時間系（曜日・時間帯・月末月初・季節イベント）、金額系（対数変換）、テキスト系（取引説明文のTF-IDFベクトル化）、カテゴリ系、交互作用系（高額取引×月末タイミング）の5種を設計',
        '【承認時期予測モデル開発】リーク除去後のテスト1万件で、承認月の正解率92.80%・日数の誤差（MAE）2.54日。高・中・低の3段階信頼度スコアを導入して出力を振り分け、低信頼度の予測にはルール補正とセカンダリモデルによる再予測を適用。高信頼度の8,408件では正解率95.96%・MAE 1.57日。信頼度別にモデルを分ける案も検証したが、単一モデルの方が高精度だったため採用しなかった',
        '【データパイプライン構築】BigQueryのスキーマ定義・テーブル生成・重複排除・差分挿入からなる品質保証パイプラインを構築し、週次でスプレッドシートへ自動反映して経理業務の工数を削減',
      ],
      achievements:
        '承認されるかの予測は精度95.7%・AUC 0.940（社内データのテスト2万件）。承認月の予測は正解率92.80%（テスト1万件）',
      techStack: [
        'Python',
        'LightGBM',
        'scikit-learn',
  ...
  },
  {
    ...
    summary: {
      ...
      result: '4日間のインターンシップで優秀者に選出され、後日 COO・CTO との特別面談に招待された。',
    },
    short: true,
    company: '株式会社スタンバイ',
    role: 'MLエンジニア',
    ...
    description:
      '求人検索サービスの実データを使い、課題の分析からAIによる解決策の実装、CTOへの発表までを4日間で実施。優秀者に選出された。',
    ...
    details: {
      ...
      achievements:
        'インターンシップ優秀者に選出。後日COO・CTOとの特別面談に招待された。MLによる課題解決とエンジニア視点での企画を短期間で経験',
  ...
  },
  {
    ...
    details: {
      ...
      responsibilities: [
        ...
        '【温泉旅館の温度管理ハッカソン】社内コミュニティ（PlayGround）の3日間のハッカソンに6人チームで参加。温度センサーと電磁弁を Raspberry Pi につなぎ、Flutter アプリから温泉の温度を確認・調整する試作品を作った（受賞なし）',
      ],
      achievements:
      ...
      projects: [
        ...
        {
          name: '温泉旅館の温度管理を自動化するハッカソン',
          client: '',
          category: 'ハッカソン',
          period: '2021年12月',
          icon: '♨️',
          summary: '社内コミュニティ（PlayGround）の3日間のハッカソンに6人チームで参加し、旅館の温泉の温度をスマホで確認・調整する試作品を作った。',
          details: [
            '旅館では、客が手で水を出して温度を調整し、旅館の人が見回って温度を確認していた。この手間をなくすことを目指した。',
            '温度センサーと電磁弁を Raspberry Pi につなぎ、Flutter アプリと WebSocket で通信して操作できるようにした。',
          ],
          impact: '温度の取得と電磁弁の開閉をデモで示した（受賞なし）。',
          techStack: ['Python', 'Flutter', 'Raspberry Pi', 'WebSocket'],
        },
      ],
      techStack: [
        'Python',
