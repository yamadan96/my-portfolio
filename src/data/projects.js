...
const projects = [
  ...
  // 数値と限定表現（「社内評価データでの値」「PoC」）は経歴データの記載をそのまま移したもので、新しい事実は含まない
  {
    ...
    description:
      '束ねた鉄パイプの本数を画像から数える AI の PoC（試作検証）。エンジニア兼テックリードとして、モデル選定から学習・評価までを担当した。',
    headline: '従来：撮影〜計算で1枚8秒・束単位の計数が不安定 → エンジニア兼テックリード → YOLOv8＋3段階学習 → 束単位96.6%・推論0.2秒/枚（社内評価）',
    ...
    story: {
      ...
      role: 'エンジニア兼テックリードとして、モデル選定から学習・評価までを担当した。',
    ...
    technical: [
      { label: '体制と出発点', body: 'エンジニア兼テックリードとしてPoCを担当。既存システムは1本単位では9割強とされたが束単位の計数は安定せず、撮影から計算まで1枚8秒を要していた。' },
