...
const projects = [
  ...
  // 数値と限定表現（「社内評価データでの値」「PoC」）は経歴データの記載をそのまま移したもので、新しい事実は含まない
  {
    ...
    headline: '従来：撮影〜計算で1枚8秒・束単位の計数が不安定 → 4名チームのテックリード → YOLOv8＋3段階学習 → 束単位96.6%・推論0.2秒/枚（社内評価）',
    ...
    technical: [
      { label: '体制と出発点', body: 'PM1名＋エンジニア3名の4名体制で、テックリードとしてPoCを担当。既存システムは1本単位では9割強とされたが束単位の計数は安定せず、撮影から計算まで1枚8秒を要していた。' },
