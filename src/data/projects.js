...
const projects = [
  {
    ...
    image: null,
    summary: {
      built: '大規模言語モデルを日本語の指示に沿って答えられるように追加学習する仕組み（LoRA）を、PEFT などの LoRA ライブラリを使わず PyTorch で自前実装した。',
      ...
      result: 'LoRA の中核機構（線形層の差し替えとモデルへの適用）を PEFT を使わずに実装して動作。学習前の時点でモデルの出力が変わらないことを数式レベルで保証する初期化も再現できた。',
    },
    technical: [
  ...
  },
  {
    ...
    summary: {
      ...
      problem: '災害直後の被害調査は専門家が現地で目視するため時間がかかる。研究で作った分類モデルを、誰でも試せる形にした。',
    ...
    technical: [
      { label: 'モデル構成', body: 'DINOv2 をバックボーンとし、LoRA で災害画像ドメインへ適応させた分類器。DINOv2 ViT-L/14＋LoRA（r=16）にマルチタスク用の補助ヘッドを加えた学習済みモデル（単一シード）を使用。FIT2025 の評価値（Macro F1 など）は別の checkpoint・評価条件で得たもので、このアプリの精度を示すものではない。' },
      ...
      { label: '実装', body: 'Gradio による画像アップロード UI。研究コードの推論部分を移植して使用している。' },
    ],
    diagram: {
  ...
  // 数値と限定表現（「社内評価データでの値」「PoC」）は経歴データの記載をそのまま移したもので、新しい事実は含まない
  {
    ...
    description:
      '束ねた鉄パイプの本数を画像から数える AI の PoC（試作検証）。4名体制のエンジニア兼テックリードとして、モデル選定から学習・評価までを担当した。',
    headline: '従来：撮影〜計算で1枚8秒・束単位の計数が不安定 → 4名チームのエンジニア兼テックリード → YOLOv8＋3段階学習 → 束単位96.6%・推論0.2秒/枚（社内評価）',
    ...
    story: {
      problem: '束ねた鉄パイプの本数を画像から数えたい。既存システムは1本単位では9割強とされたが（キックオフ時の共有値）、束全体を正しく数えられる割合は安定せず、撮影から計算までに1枚8秒かかっていた。',
      constraints: '顧客提供の画像は341枚と少なく、公開データを加えて学習した。PoC（試作検証）であり、評価は社内評価データで行っている。',
      role: 'PM1名＋エンジニア3名の4名体制で、エンジニア兼テックリードとしてモデル選定から学習・評価までを担当した。',
    ...
    technical: [
      { label: '体制と出発点', body: 'PM1名＋エンジニア3名の4名体制で、エンジニア兼テックリードとしてPoCを担当。既存システムは1本単位では9割強とされたが束単位の計数は安定せず、撮影から計算まで1枚8秒を要していた。' },
      { label: 'データ', body: '提供画像341枚とその切り出し画像341枚に、公開データ3,809枚を加えた計4,491枚で学習。コントラスト変更とガウシアンノイズによるデータ拡張は別モデルで検証した。' },
    ...
    diagram: {
      ...
      chart: `flowchart LR
  DATA["提供画像 341枚＋切り出し 341枚<br/>＋公開データ 3,809枚"] --> TRAIN["3段階の学習<br/>事前学習 → 浅層固定 → 全層再学習"]
  TRAIN --> DET
  ...
  },
  {
    ...
    featured: false,
    category: 'product',
    title: 'AWS 3層構成の構築（実務形式の課題・検収完了）',
    description:
      '実務のロールプレイ形式で、東京リージョンに VPC・EC2 2台・ALB・RDS の3層構成を構築。発注者役への提出と検収までを一人で担当し、2026年8月に完了した。',
    evidence: 'VPC・EC2 2台・ALB・RDS を構築 · 検証16項目すべて合格 · 検収完了',
    tags: ['AWS', 'VPC', 'EC2', 'ALB', 'RDS'],
    github: null,
    demo: null,
    image: null,
    summary: {
      built: 'Web サーバー2台とデータベースを、負荷を振り分ける装置の後ろに並べた構成を AWS 上に作った。',
      problem: '受講者が発注者役とやり取りしながら進める形式で、要件の確認から構築・提出・検収までを自分で回す必要があった。',
      role: '構築、検証、提出、差し戻し対応、費用の実測、リソース削除までを一人で担当。',
      tech: '東京リージョンに VPC を作り、公開側にロードバランサー、非公開側に EC2 2台とデータベースを配置。検証スクリプトで16項目を確認した。',
      result: '検証16項目すべて合格。ヘルスチェックで2台とも正常であることを示して検収完了。費用は Cost Explorer で実測し、終了後にリソースを削除した。',
    },
    technical: [
      { label: '構成', body: '東京リージョンの VPC 内に、公開サブネットへ ALB（負荷分散装置）、非公開サブネットへ EC2 2台と RDS（データベース）を配置した3層構成。' },
      { label: '検証', body: '構成を確認するスクリプトで16項目を点検し、すべて合格した状態で提出した。' },
      { label: '差し戻しとその原因', body: '提出後に「スクリプトは設定値しか見ておらず、実際に正常かは判定されない」と差し戻された。確認したところリージョンの切り替えを戻し忘れていたため、東京リージョンでターゲット2台が正常であることを示して再提出し、検収を通した。' },
      { label: '学んだこと', body: '提出物の形式について不明点があると気づいていながら、確認せずに出したことが差し戻しの原因だった。気づいた時点で発注者役に聞いておけば防げた。' },
      { label: '後片付け', body: '検収後に ALB、ターゲットグループ、RDS の順で削除し、最終スナップショットを残した。費用は Cost Explorer で実測して記録した。' },
    ],
    diagram: {
      alt: 'AWS の東京リージョンに VPC を作り、ロードバランサーの後ろに EC2 2台とデータベースを置いた3層構成の図',
      caption: '公開側にロードバランサー、非公開側に Web サーバー2台とデータベースを置く',
      chart: `flowchart TB
  USER["利用者"] --> ALB
